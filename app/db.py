...
def init_db() -> None:
    ddl = """
    CREATE TABLE IF NOT EXISTS payhub_users (
    ...
    CREATE TABLE IF NOT EXISTS payhub_bots (
        id BIGSERIAL PRIMARY KEY,
    ...
    CREATE TABLE IF NOT EXISTS payhub_invoices (
    ...
    CREATE TABLE IF NOT EXISTS payhub_used_tx (
        txid TEXT PRIMARY KEY,
        invoice_id TEXT NOT NULL,
        used_at TEXT NOT NULL
    );
    CREATE TABLE IF NOT EXISTS payhub_webhook_logs (
        id BIGSERIAL PRIMARY KEY,
        bot_id BIGINT,
        event TEXT,
        payload TEXT,
    ...
    CREATE TABLE IF NOT EXISTS payhub_messages (
    ...
    CREATE INDEX IF NOT EXISTS idx_payhub_bots_user_id ON payhub_bots(user_id);
    CREATE INDEX IF NOT EXISTS idx_payhub_invoices_user_created ON payhub_invoices(user_id, created_at DESC);
    CREATE INDEX IF NOT EXISTS idx_payhub_invoices_bot_id ON payhub_invoices(bot_id);
    """
...
def get_messages(user_id: int) -> dict:
    with connect() as (_, cur):
        cur.execute("SELECT * FROM payhub_messages WHERE user_id=%s", (user_id,))
        row = cur.fetchone()
...
def save_messages(user_id: int, deposit_text: str, success_text: str, fail_not_found: str, fail_mismatch: str) -> None:
    with connect() as (_, cur):
        cur.execute(
            """INSERT INTO payhub_messages(user_id,deposit_text,success_text,fail_not_found,fail_mismatch)
               VALUES(%s,%s,%s,%s,%s)
...
def create_user(email: str, password: str) -> dict[str, Any]:
    ...
    with connect() as (_, cur):
        cur.execute(
            "INSERT INTO payhub_users(email,password_hash,status,approve_token,created_at) VALUES(%s,%s,%s,%s,%s) RETURNING id",
...
def get_user_by_email(email: str) -> Optional[dict]:
    with connect() as (_, cur):
        cur.execute("SELECT * FROM payhub_users WHERE email=%s", (email.lower().strip(),))
        return cur.fetchone()


def get_user(user_id: int) -> Optional[dict]:
    with connect() as (_, cur):
        cur.execute("SELECT * FROM payhub_users WHERE id=%s", (user_id,))
        return cur.fetchone()


def get_user_by_token(token: str) -> Optional[dict]:
    with connect() as (_, cur):
        cur.execute("SELECT * FROM payhub_users WHERE approve_token=%s", (token,))
        return cur.fetchone()


def set_status(user_id: int, status: str) -> None:
    with connect() as (_, cur):
        cur.execute("UPDATE payhub_users SET status=%s WHERE id=%s", (status, user_id))


def list_users() -> list[dict]:
    with connect() as (_, cur):
        cur.execute("SELECT * FROM payhub_users ORDER BY id DESC")
        return cur.fetchall()


def pending_users() -> list[dict]:
    with connect() as (_, cur):
        cur.execute("SELECT * FROM payhub_users WHERE status='pending' ORDER BY id DESC")
        return cur.fetchall()
...
def save_binance(user_id: int, key: str, secret: str, uid: str, name: str) -> None:
    with connect() as (_, cur):
        cur.execute(
            "UPDATE payhub_users SET binance_api_key=%s, binance_api_secret=%s, binance_uid=%s, binance_name=%s WHERE id=%s",
...
def create_bot(user_id: int, name: str, webhook_url: str = "") -> dict[str, Any]:
    ...
    with connect() as (_, cur):
        cur.execute(
            """INSERT INTO payhub_bots(user_id,name,api_key,webhook_url,webhook_secret,created_at)
...
def list_bots(user_id: int) -> list[dict]:
    with connect() as (_, cur):
        cur.execute("SELECT * FROM payhub_bots WHERE user_id=%s ORDER BY id DESC", (user_id,))
        return cur.fetchall()


def get_bot(bot_id: int) -> Optional[dict]:
    with connect() as (_, cur):
        cur.execute("SELECT * FROM payhub_bots WHERE id=%s", (bot_id,))
        return cur.fetchone()


def get_bot_by_key(api_key: str) -> Optional[dict]:
    with connect() as (_, cur):
        cur.execute("SELECT * FROM payhub_bots WHERE api_key=%s AND active=1", (api_key,))
        return cur.fetchone()


def update_bot_webhook(bot_id: int, user_id: int, webhook_url: str) -> None:
    with connect() as (_, cur):
        cur.execute("UPDATE payhub_bots SET webhook_url=%s WHERE id=%s AND user_id=%s", (webhook_url.strip(), bot_id, user_id))


def create_invoice(invoice_id: str, user_id: int, bot_id: int, telegram_id: str, amount: str, currency: str) -> None:
    with connect() as (_, cur):
        cur.execute(
            """INSERT INTO payhub_invoices(invoice_id,user_id,bot_id,telegram_id,amount,currency,status,created_at)
...
def get_invoice(invoice_id: str) -> Optional[dict]:
    with connect() as (_, cur):
        cur.execute("SELECT * FROM payhub_invoices WHERE invoice_id=%s", (invoice_id,))
        return cur.fetchone()
...
def mark_paid(invoice_id: str, txid: str) -> None:
    with connect() as (_, cur):
        cur.execute(
            "UPDATE payhub_invoices SET status='PAID', txid=%s, paid_at=%s WHERE invoice_id=%s AND status!='PAID'",
            (txid, utcnow(), invoice_id),
        )
        cur.execute(
            "INSERT INTO payhub_used_tx(txid,invoice_id,used_at) VALUES(%s,%s,%s) ON CONFLICT(txid) DO NOTHING",
...
def tx_used(txid: str) -> bool:
    with connect() as (_, cur):
        cur.execute("SELECT 1 FROM payhub_used_tx WHERE txid=%s", (txid,))
...
def claim_payment(invoice_id: str, txid: str) -> tuple[bool, str]:
    ...
    try:
        ...
        with con.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("SELECT * FROM payhub_invoices WHERE invoice_id=%s FOR UPDATE", (invoice_id,))
            inv = cur.fetchone()
            if not inv:
                con.rollback()
            ...
            cur.execute("SELECT invoice_id FROM payhub_used_tx WHERE txid=%s FOR UPDATE", (txid,))
            used = cur.fetchone()
            if used:
                con.rollback()
            ...
            try:
                cur.execute("INSERT INTO payhub_used_tx(txid,invoice_id,used_at) VALUES(%s,%s,%s)", (txid, invoice_id, utcnow()))
            except psycopg2.errors.UniqueViolation:
                con.rollback()
                return False, "tx_already_used"

            cur.execute(
                "UPDATE payhub_invoices SET status='PAID', txid=%s, paid_at=%s WHERE invoice_id=%s AND status='PENDING'",
                (txid, utcnow(), invoice_id),
            )
            if cur.rowcount != 1:
                con.rollback()
...
def list_invoices(user_id: int, limit: int = 50) -> list[dict]:
    with connect() as (_, cur):
        cur.execute(
            "SELECT * FROM payhub_invoices WHERE user_id=%s ORDER BY created_at DESC LIMIT %s",
            (user_id, limit),
        )
        return cur.fetchall()
